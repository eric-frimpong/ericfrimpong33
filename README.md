...
| 🌐 **DNS Server Role** | Installed and configured the DNS Server role, created Forward and Reverse Lookup Zones, added A records and PTR records, and verified name resolution from client VMs using `nslookup` |
| 📡 **DHCP Server Role** | Deployed the DHCP Server role, created an IP address scope with exclusions for static devices, set lease duration, and configured DHCP options (Default Gateway, DNS Server) for automatic client configuration |
| 🏢 **Active Directory Domain Services (AD DS)** | Installed AD DS, promoted the server to a Domain Controller, created the domain, and configured the forest/domain functional levels |
| 👤 **User & Group Management** | Created Organizational Units, user accounts, and security groups in Active Directory Users & Computers; set account properties, passwords, and group memberships |
